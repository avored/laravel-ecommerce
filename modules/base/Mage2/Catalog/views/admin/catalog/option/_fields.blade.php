...
@if(!isset($option->validation))
    <?php $validations = []; ?>
@else
    <?php $validations = explode("|", $option->validation); ?>
@endif
...
$hiddenClass = "hidden";
$editMode = false;
if (isset($option) && $option->optionDropdownValues->count() > 0) {
    $editMode = true;
    $hiddenClass = "";
}
?>
...
<div class="dynamic-field {{ $hiddenClass }}">
    ...
    @if($editMode === true)
        ...
        @foreach($option->optionDropdownValues as $key => $dropdownOptionModel)
